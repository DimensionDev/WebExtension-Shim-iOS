	...
	***************************************************************************** */
	...
	class WebExtensionEnvironment {
	    constructor(extensionID, manifest) {
	        const realm = realmsShim_umd.makeRootRealm();
	...
	WebExtensionEnvironment.staticGlobal = (() => {
	    const obj = Object.getOwnPropertyDescriptors(globalThis);
	    for (const key in obj) {
	        const desc = obj[key];
	        const { get, set, value } = desc;
	        if (get)
	            desc.get = () => get.apply(globalThis);
	        if (set)
	            desc.set = (val) => set.apply(globalThis, val);
	...
	const extension = new WebExtensionEnvironment('FA7A4142-B555-45CE-87A5-E6E8AEA39A61', JSON.parse('{}'));
